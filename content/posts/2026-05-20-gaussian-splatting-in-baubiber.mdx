...
coverImage: "/images/posts/gaussian_splatting.jpg"
draft: false
series: "baubiber"
tags:
  - baubiber
...
The technique is called Gaussian splatting. The paper that made it run in real time is from 2023, which in this corner of the world is geological. The idea, roughly: instead of representing a 3D scene as triangles and textures, you represent it as something like a million little fuzzy ellipsoids floating in space, each with a position, colour, opacity, and a stretched shape, which you then sort back-to-front and paint onto the screen. A counter is not a modeled object in this setup. It is a few thousand blobs that happen to align when you look from where I was standing.

It is, by any sane standard, an absurd way to do graphics. It also happens to solve the job in front of me: a contractor in Pécs needs to see a kitchen in Tamarindo without flying there, and a photograph isn't enough.
...
You can crank the photo count and the messaging frequency forever and you'll still be losing information in the compression, because the room is volumetric and the medium isn't. A walkable splat closes that gap badly, but well enough that a contractor can bid without first driving an hour each way to see the site.

## How bad the splats actually are

The splats are quite bad, in ways that get more interesting once you start poking at them.
...
I love them anyway, because the bad bits are the kind that papers are already chewing through. Every new release either compresses the splats, generates them from fewer photos, or patches the holes the current ones leave behind. The room keeps getting lighter to send and easier to trust.
